...
function submitCoupon() {
  const couponData = { ...tempCoupon.value }

  // 將日期字符串轉換為時間戳（秒）
  if (couponData.due_date) {
    couponData.due_date = Math.floor(
      new Date(couponData.due_date).getTime() / 1000
    )
  }

  emit('update-coupon', couponData)
}
</script>
  ...
  <div class="absolute inset-0 bg-black/70 z-11" v-if="isCouponModal">
    <section class="max-w-[500px] mx-auto absolute inset-70 z-100">
      <div class="bg-white rounded-lg">
        <h2
          class="text-center text-2xl text-white bg-sky-700 py-3 rounded-t-md"
        >
          新增優惠券
        </h2>
      </div>
      <div class="bg-white rounded-b-lg flex flex-col gap-5 p-5">
        ...
        <div class="flex justify-center gap-3">
          <button
            class="bg-red-700 hover:bg-red-600 px-3 py-2 rounded-lg flex items-center justify-center w-[20%]"
            @click="emit('close-coupon-modal')"
          >
            <span class="text-xl text-white">取消</span>
          </button>
          <button
            class="bg-sky-700 hover:bg-sky-600 px-3 py-2 rounded-lg flex items-center justify-center w-[20%]"
            @click="submitCoupon()"
          >
